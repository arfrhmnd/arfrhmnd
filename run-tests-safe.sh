...
# PHTN.ai Automation Framework - Safe Test Runner
# Script yang aman untuk menjalankan automation tests dengan path yang mengandung spasi
...
run_tests() {
    ...
    CMD_ARGS=("npx" "wdio" "run" "wdio.conf.js")
    ...
    # Execute command with proper array handling
...
generate_report() {
    if [ "$REPORT" = true ]; then
        ...
        print_status "Generating report from: $(pwd)/allure-results"
        print_status "Output directory: $(pwd)/allure-report"
        ...
        if [ $? -eq 0 ]; then
            print_success "Allure report generated successfully at: $REPORT_DIR"
...
# Main execution
main() {
    print_status "PHTN.ai Automation Framework Test Runner (Safe Mode)"
    print_status "=================================================="
